...
class ORGANIZER:
    def __init__(self, path):
        ...
        for i in EXTENCOES_DICIONARIO:
            self.CRIAR_DIRETORIOS(path, i)
            self.ORGANIZA_PASTA(path, EXTENCOES_DICIONARIO[i], i)
            # ORGANIZA_PASTA(diretorio_atual,EXTENCOES_DICIONARIO[i],i)
    
        PRINT_LAYER()
        time.sleep(1)
    
        self.REMOVER_DIRETORIO_VAZIO(path)
        PRINT_LAYER()

        print()  
    
    def CRIAR_DIRETORIOS(self, diretorio, nome_pasta):
        try:
            if nome_pasta == "":
    ...
    def REMOVER_DIRETORIO_VAZIO(self, diretorio):
        PASTAS_LISTA = []
    ...
    def ORGANIZA_PASTA(self, diretorio, lista_extensoes, nome_pasta):
